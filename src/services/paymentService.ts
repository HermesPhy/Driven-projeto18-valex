...
export default async function paymentService({cardId, password, businessId, value}: paymentData) {
    ...
    if (!card.password) throw {type: 'invalid_card_error', message: "Cartão não ativo", statusCode: 401};
    ...
    if (!business) throw {type: 'business_not_found', message: "Comércio não encontrado", statusCode: 404};

    if (business.type !== card.type) throw {type: 'card_type_error', message: "Cartão inválido para este comércio", statusCode: 403};
